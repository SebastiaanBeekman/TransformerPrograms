...
def run(tokens):
    ...
    classifier_weights = pd.read_csv(
        "output/length/rasp/hist/trainlength30/s2/hist_weights.csv",
        index_col=[0, 1],
        dtype={"feature": str},
    )
    # inputs #####################################################
    ...
    def num_mlp_0_1(num_attn_0_3_output):
        key = num_attn_0_3_output
        if key in {32, 33, 34, 35, 36, 37, 38, 39}:
            return 18
        elif key in {0, 1}:
            return 19
        elif key in {2, 3}:
