...
def run(tokens):
    ...
    classifier_weights = pd.read_csv(
        "output/length/rasp/hist/trainlength20/s2/hist_weights.csv",
        index_col=[0, 1],
        dtype={"feature": str},
    )
    # inputs #####################################################
    ...
    def predicate_0_1(position, token):
        if position in {0, 3, 13, 20, 21, 22, 24, 26}:
            return token == ""
        elif position in {1, 2, 7, 10, 14, 18}:
            return token == "4"
        elif position in {4, 5, 8, 9, 11, 12, 16, 23, 25, 27, 28, 29}:
            return token == "2"
        elif position in {19, 6, 15}:
            return token == "1"
        elif position in {17}:
            return token == "<s>"
